 ...
 **
 **【重要说明】  本文件的收发机制, 经多次修改, 已比较完善. 
 **              初始化: 只需调用：USARTx_Init(波特率), 函数内已做好引脚及时钟配置； 
 **              发 送 : 两个函数, 字符串: USARTx_printf (char* stringTemp)、 数据: USARTx_SendData (uint8_t* buf, uint8_t cnt);
 **              接 收 : 两种方式, 1-USARTx_GetBuffer (uint8_t* buffer, uint8_t* cnt); 2-判断xUSART.USARTxRecivedFla==1;
 **
 **
